...
//The drawing contains only the x axis (drawn with the code below), the y axis isn't drawn because it is not required and it is not necessary
...
heightX = height - 100; //separate the bottom of svg to x axis; heightX is the height from top to x axis
function drawXaxis(){
	svg.append("g")
		   ...
		   .call(xAxis)
		   .attr("font-size", "12")
		   .attr("text-anchor","end") //numbers of ticks aligned on left side
...
//roots
var scaleRoot = d3.scaleLinear().range([20, 60]); //scale root height -> the roots don't exceed a certain height;
...
function drawingTrees(dataSet, position, label){
	...
	//draw description of the values on the x axis
	svg.append("text")
	...
	svg.append("text").attr("class", "lab")
       ...
       .style("font-weight", 700)
       .attr("fill", "blue")
       .text(label); //the firs value of label is "default position"; then label shows wich feature is selected
	...
	function treePositionXaxis(positionTree){
		...
		currentPos = arrayPosition[counterX];
		counterX = counterX + 1; //increase the index used to extract position
		currentPos = xScale(currentPos);
		return currentPos;
	}
	...
	var counterX = 0; //counter for extract x coordinate from array position, it is set to zero whenever a new element is drawn 
	...
	scaleCrownArea.domain([1500, d3.max(dataSet, function(d) { return d.crownArea; })]); // Set the input domain
	...
	scaleFruitArea.domain([10, d3.max(dataSet, function(d) { return d.fruitArea; })]); // Set the input domain
	...
	//draw first fruit, counter set to zero to re-iterate x position array
	counterX = 0;
	...
	//draw second fruit, counter set to zero to re-iterate x position array
	counterX = 0;
	...
	//draw first root, counter set to zero to re-iterate x position array
	counterX = 0; 
